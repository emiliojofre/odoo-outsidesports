...
try:
    ...
    # FIX NLH: maxsize>1 evita "Connection pool is full, discarding connection"
    # cuando hay varios pagos concurrentes o reintentos del usuario.
...
class Client(object):
    ...
    def __make_signature(self, method, url, params=None, data=None):
        ...
        if self.is_debug:
            pattern = r"(apiKey=)([A-Za-z0-9]{8})-([A-Za-z0-9]{4})-([A-Za-z0-9]{4})-([A-Za-z0-9]{4})-([A-Za-z0-9]{12})"
            _logger.warning("A firmar: {}".format(re.sub(pattern, "\1********-****-****-****-\6", to_sign)))
    ...
    def make_request(self, method, endpoint, params=None, data={}):
        ...
        # FIX NLH: timeout explícito (antes no existía y la request podía
        # quedar colgada hasta que Flow/el proxy cortaran con 504).
        timeout = urllib3.Timeout(connect=5.0, read=20.0)
        try:
            response = pool.request(
                method, url, values, timeout=timeout, retries=False
            )
        except urllib3.exceptions.HTTPError as e:
            _logger.warning(u"Error de conexión con Flow: {0}".format(e))
            raise
