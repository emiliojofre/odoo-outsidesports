...
$(document).ready(function () {
    ...
    let len_sku = $('.sku').length;
    console.log(len_sku);
    if (len_sku > 1) {
        $('h4:first').css("display", "none");
    } else {
        $('h4:first').css("display", "block");
    }

    $('.sku').css("display", "none");

    var variant = $(".js_variant_change");
    var type = variant.attr('type');

    console.log(type, 'Tipo');
    console.log(variant, 'Variante');

    variant.on('change', function (ev) {
        // ev.stopPropagation();
        $('.sku').css("display", "none");
        $('.sku').removeAttr("id");
        ...
        $("input[type='radio']:checked").each(function (idx) {
            arrayAttributes.push(parseInt($(this).val()));
        });
        arrayAttributes.push(parseInt($("option:checked").val()));
        arrayAttributes = arrayAttributes.filter(value => !Number.isNaN(value));
        arrayAttributes = arrayAttributes.sort()
        $('.sku').each(function (idx) {
            let attr = $(this).data('attibutes-val');
            if (attr.length === 1) {
                if ((arrayAttributes[0] === attr[0] && arrayAttributes[1] === attr[1]) || (arrayAttributes[0] === attr[1] && arrayAttributes[1] === attr[0])) {

                    if ($(this).is("a")) {
                        $(this).css("display", "inline-block");
                        $(this).attr('id', 'add_to_cart')
                    } else {
    ...
    var FirstMenuCategoryLi = $('.FirstMenuCategory > ul > li');
    console.log('Los li', FirstMenuCategoryLi);

    FirstMenuCategoryLi.hover(function () {
        $('[id^="parent_"]').css("display", "none");

        var SecondMenuCategory = $('#parent_' + $(this).attr("id"));
        console.log(SecondMenuCategory, 'Menu secundario');
        SecondMenuCategory.css("display", "block");
    });

    var SpanHide = $('.hidediv');
    console.log(SpanHide, 'SPAN');
    SpanHide.each(function (idx) {
