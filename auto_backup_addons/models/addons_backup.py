...
class addons_backup(models.Model):
    _name = 'addons.backup'
    _description = "Addons Backup"
    ...
    host = fields.Char('Hostname', size=100, required=True, default='localhost')
    ...
    folder_sourcecode = fields.Char('Source code Directory', size=100, help='Absolute path for source code to be backuped', required='True', default='/opt/odoo')
    backup_folder = fields.Char('Destination Directory', size=100, help='Absolute path for storing the backups', required='True', default='/odoo/backups')
    ...
    def test_sftp_connection(self, context=None):
        ...
        for rec in self:
            ...
            try:
                ...
                srv = pysftp.Connection(host=ipHost, username=usernameLogin, password=str(passwordLogin),port=portHost)
                srv.close()
                #We have a success.
    ...
    def schedule_backup(self):
        ...
        for rec in conf_ids:
            ...
            if rec.name in db_list:
                try:
                    if not os.path.isdir(rec.backup_folder):
                        os.makedirs(rec.backup_folder)
                    if not os.path.isdir(rec.folder_sourcecode):
                        os.makedirs(rec.folder_sourcecode)
                except:
                    raise

                folder_name = os.path.basename(rec.folder_sourcecode)
                bkp_source_file='(%s)_%s-%s.%s' % (time.strftime('%d-%m-%Y_%H:%M:%S'),folder_name,rec.name, 'zip')
                source_file_path = os.path.join(rec.folder_sourcecode,'')
                backup_file_path = os.path.join(rec.backup_folder,bkp_source_file)
                try:
                    try:
                        ...
                        zipdir(source_file_path, zipf)
                        zipf.close()
                    except:
                        _logger.debug("Couldn't backup source code")
                        continue
                except:
                    _logger.debug("Couldn't backup source code")
                    continue
            ...
            if rec.sftp_write is True:
                try:
                    ...
                    for f in os.listdir(dir):
                        if rec.name and os.path.basename(rec.folder_sourcecode) in f:
            ...
            if rec.autoremove:
                ...
                for f in os.listdir(dir):
                    ...
                    if rec.name and os.path.basename(rec.folder_sourcecode) in fullpath:
