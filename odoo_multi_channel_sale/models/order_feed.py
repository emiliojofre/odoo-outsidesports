    ...
    def import_partner(self,channel_id):
        ...
        if parent_store_id:
            ...
            if partner_id:
                ...
                _logger.info('........ERROR: %r .............', partner_id)
                state = 'error'
        if state == 'done':
            ...
            if last_name:
                ...
                vals['name'] =name
        _logger.info('........match mapping: %r .............', match)
        if match:
            if  state =='done' :
                try:
                    _logger.info('........Feed Write 1:.............')

                    res = match.odoo_partner.write(vals)
                    _logger.info('........Feed Write 2: %r .............', res)

                    message +='<br/> Partner %s successfully updated'%(name)
                except Exception as e:
                    message += '<br/>%s' % (e)
                    _logger.info('........ERROR1: %r .............', message)
                    state = 'error'
                update_id = match
...
class OrderFeed(models.Model):
    ...
    def _get_order_line_vals(self, vals,carrier_id,channel_id):
        ...
        if line_ids:
            for line_id in self.env['order.line.feed'].browse(line_ids):
                ...
                if line_id.line_source =='delivery':
                    ...
                    product_res = self.get_product_id(
                        ...
                        int(line_id.line_variant_ids) or 'No Variants', #To fix issue with prestashop
                        channel_id,
                        line_id.line_product_default_code,
                        line_id.line_product_barcode,

                    )
                    _logger.info("======product res %r",(line_price_unit,product_res))
            ...
            product_res = self.get_product_id(
                line_product_id,
                int(line_variant_ids) or 'No Variants',
                channel_id,
                line_product_default_code,
                line_product_barcode,
