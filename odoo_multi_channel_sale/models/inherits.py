...
class ResPartner(models.Model):
    ...
    def create(self, vals):
        if 'street2' in vals and 'country_id' in vals and not vals.get('city_id') :
            city_id = self.env['res.city'].search([
                ('name', '=', vals['street2']),
                ('country_id', '=', vals['country_id'])], limit=1)
            if city_id and city_id.id:
                vals['city_id'] = city_id.id
            else:
                city_id = self.env['res.city'].create({
                    'name': vals['street2'],
                    'country_id':vals['country_id'],
                    'code':vals['street2'][:2].upper()
                    })
                vals['city_id'] = city_id.id
        # _logger.info('.......Vals: %r .......', vals)
        temp = super(ResPartner, self).create(vals)
        # if 'parent_id' in vals and "street2" in vals:
        #     res_partner = self.env['res.partner'].browse(vals['parent_id'])
        #     if res_partner.child_ids:
        #         first_child_vals = res_partner.child_ids[0].read(["street","street2","city","state_id","zip","country_id","city_id"])[0]
        #         first_child_vals.pop("id")
        #         first_child_vals["country_id"] = first_child_vals.get("country_id")[0]
        #         first_child_vals["state_id"] = first_child_vals.get("state_id")[0]
        #         first_child_vals["city_id"] = first_child_vals.get("city_id")[0]
        #
        #         res_partner.write(first_child_vals)
        return temp

    @api.multi
    def write(self, vals):
        # not vals.get("city_id") or 
        _logger.info('.......vals: %r .......', vals)

        if 'street2' in vals and 'country_id' in vals: 
            if vals.get('street2')!=False and vals.get('country_id')!=False:
                city_id = self.env['res.city'].search([
                    ('name', '=', vals['street2']),
                    ('country_id', '=', vals['country_id'])], limit=1)
                if city_id and city_id.id:
        ...
        # for rec in self:
        #     res_partner = rec.parent_id
        #     if res_partner and not res_partner.street2:
        #         if res_partner.child_ids:
        #             first_child_vals = res_partner.child_ids[0].read(["street","street2","city","state_id","city_id","zip","country_id"])[0]
        #             first_child_vals.pop("id")
        #             # first_child_vals.pop("parent_id")
        #             if first_child_vals.get("country_id"):
        #                 first_child_vals["country_id"] = first_child_vals.get("country_id")[0]
        #             if first_child_vals.get("state_id"):
        #                 first_child_vals["state_id"] = first_child_vals.get("state_id")[0]
        #             if first_child_vals.get("city_id"):
        #                 first_child_vals["city_id"] = first_child_vals.get("city_id")[0]
        #             # _logger.info('.......first_child_vals: %r .......', first_child_vals)
        #             res_partner.write(first_child_vals)
        temp = super(models.Model, self).write(vals)
        return temp
