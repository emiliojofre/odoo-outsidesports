...
class WkFeed(models.Model):
    ...
    def get_product_id(self, store_product_id, line_variant_ids, channel_id ,default_code=None,barcode = None):
        ...
        _logger.info("===== product info %r",(store_product_id,line_variant_ids,default_code,barcode,match))
        if match:
...
class ProductFeed(models.Model):
    ...
    def update_product_variants(self, variant_ids, template_id, store_id, location_id,channel_id):
        ...
        for variant in variant_objs:
            ...
            if not exists:
                ...
                _logger.info("=====variant cost %r",(variant_store_id,variant.standard_price,parse_float(variant.standard_price)))
                if variant.standard_price:
                    exists.product_name.standard_price = parse_float(variant.standard_price)
    ...
    def _create_product_line(self, variant, template_id, store_id, location_id,channel_id):
        ...
        _logger.info("=========_create_product_line %r",(store_id, variant_id))
        if not exists:
            vals = {
            ...
            vals['standard_price'] = parse_float(variant.standard_price)
    ...
    def import_product(self,channel_id):
        ...
        if not store_id:
            ...
            state = 'error'
        temp = self.get_categ_id(vals.pop('id_categ_default'),channel_id)
        categ_id = temp.get('categ_id')
        if categ_id:
        ...
        vals.pop('website_message_ids','')
        vals.pop('message_follower_ids','')
        _logger.info("=======product found %r",(match,template_exists_odoo,store_id,vals["name"]))
        if state == 'done':
            if match:
