...
class ImportPrestashopCategories(models.TransientModel):
    ...
    def _prestashop_import_category(self, prestashop, category_id):
        ...
        if match:
            ...
            if map_match:
                try:
                    map_match.category_name.write({'name':categ_vals.get('name')})
    ...
    def _get_prestashop_category_list(self, prestashop):
        ...
        if type(category_list) == dict:
            ...
            cat_ids = [i['attrs']['id'] for i in category_list]
        return dict(
            data = cat_ids,
            message = message
        )
...
class ExportPrestashopCategories(models.TransientModel):
        ...
        if cat_data:
            if type(cat_data['category']['name']['language']) == list:
                for i in range(len(cat_data['category']['name']['language'])):
                    ...
                    cat_data['category']['link_rewrite']['language'][i]['value'] = self.channel_id._get_link_rewrite(zip, name)
                ...
                cat_data['category']['link_rewrite']['language']['value'] = self.channel_id._get_link_rewrite(zip, name)
            ...
            if returnid:
                cid = returnid
                mapping_id=self.channel_id.create_category_mapping(cat_id, cid)
    ...
    def export_now(self):
        ...
        exclude_type_ids = []
        mapped = []
        already_mapped = self.env['channel.category.mappings'].search([('channel_id','=', self.channel_id.id)])
        for record in self:
            channel_id = record.channel_id
            prestashop = PrestaShopWebServiceDict(channel_id.prestashop_base_uri, channel_id.prestashop_api_key)
            ...
            cat_to_export = self.env['product.category'].search(
                [('id', 'not in', mapped)])
            if not prestashop:
                message += "Error in connection"
            if cat_to_export:
    ...
    def sync_categories(self, prestashop, cat_id, active='1'):
        ...
        if not check:
            ...
            _logger.info("=====>>>get_response %r",get_response)
            try:
                ctx['create_ids'] += get_response[3]
            except Exception as e:
