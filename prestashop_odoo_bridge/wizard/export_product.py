...
class ExportPrestashopProducts(models.TransientModel):
        ...
        if prestashop:
            ...
            for j in selected_ids:
                check = self.env['channel.product.mappings'].search(
                    [('product_name', '=', j),('channel_id','=',channel_id.id)])
                if not check:
        ...
        if prestashop:
            ...
            for j in selected_ids:
                check = self.env['channel.template.mappings'].search(
                    [('odoo_template_id', '=', j), ('channel_id','=',channel_id.id)])
                if check:
                    ...
                    if prestashop and need_update_id:
                        ...
                        if temp_obj:
                            ...
                            if temp_obj.list_price:
                                price = temp_obj.with_context(pricelist= channel_id.pricelist_name.id).list_price
                            else:
                                price = 0.0
                            if temp_obj.weight:
            ...
            message = 'Product Template(s) Updated: %s\r\nNumber of Product Template(s) Exported: %s' % (
                len(exported_ids), len(need_to_export)) + ' \r\n' + message
        return self.env['multi.channel.sale'].display_message(message)
    ...
    def export_all_products(self, prestashop):
        ...
        return {'create_ids': need_to_export,
                'message': message
                }
    ...
    def export_product(self, prestashop, product_bs, combination_bs, erp_product_id):
        ...
        if product_data.product_tmpl_id:
            ...
            check = template_obj.search(
                [('odoo_template_id', '=', erp_template_id), ('channel_id', '=', self.channel_id.id)])
            if check:
                ps_template_id = check[0].store_product_id
            else:
    ...
    def export_template(self, prestashop, product_bs, erp_template_id):
        ...
        ps_extra_categ = []
        extra_categories = self.env['extra.categories'].search([('instance_id','=',channel_id.id), ('product_id','=', erp_template_id)])
        if extra_categories:
            for categ in extra_categories.extra_category_ids:
                cat_id = self._get_store_categ_id(categ.id)
                ps_extra_categ.append({'id': str(cat_id)})
        product_bs['product'].update({
            'price': str(template_data.with_context(pricelist= channel_id.pricelist_name.id).list_price),
            'active': '1',
            'redirect_type': '404',
            'minimal_quantity': '1',
        ...
        if returnid:
            temp = channel_id.create_template_mapping(
                template_data, returnid, {'default_code': default_code, 'barcode': ''})
            return [returnid, '']
    ...
    def create_combination(self, prestashop, add_comb, presta_main_product_id, erp_template_id, erp_product_id):
        ...
        if image:
            ...
            if image_id:
                add_comb['combination']['associations']['images']['image']['id'] = str(
                    image_id)
        price_extra = float(obj_pro.with_context(pricelist= channel_id.pricelist_name.id).lst_price) - float(obj_pro.with_context(pricelist= channel_id.pricelist_name.id).list_price)
        ...
        if returnid:
            ...
            temp = channel_id.create_product_mapping(
                erp_template_id, obj_pro, presta_main_product_id, pid, {'default_code': default_code, 'barcode': ''})
    ...
    def create_normal_product(self, prestashop, erp_template_id, erp_product_id, prest_main_product_id):
        ...
        if add_data:
            add_data['product'].update({
                'price': str(product_data.with_context(pricelist= channel_id.pricelist_name.id).lst_price),
                'active': '1',
                'redirect_type': '404',
                'minimal_quantity': '1',
                'available_for_order': '1',
            ...
            if returnid:
                temp = channel_id.create_product_mapping(
                    erp_template_id, product_data, prest_main_product_id, '0', {'default_code': default_code, 'barcode': ''})
            if product_data.image:
                get = self.create_images(
                    prestashop, product_data.image, prest_main_product_id)
        ...
        if template_data:
            ...
            if returnid:
                map_id = self.env['channel.template.mappings'].search(
                    [('odoo_template_id', '=', erp_id), ('channel_id', '=', self.channel_id.id)])
                map_id.need_sync = 'no'
    ...
    def update_template_category(self, prestashop, erp_id, presta_id):
        ...
        search_cat = cat_obj.search(
            [('odoo_category_id', '=', template_obj.categ_id.id)])
        if search_cat:
            default_category = search_cat[0].store_category_id
        else:
            resp = self.env['export.prestashop.categories'].with_context({'err_msg': '', 'channel_id':self.channel_id}).sync_categories(
        ...
        ps_extra_categ = []
        # template_obj = self.env['product.template'].browse(erp_id)
        extra_categ = self.env['extra.categories'].search([('instance_id','=',self.channel_id.id), ('product_id','=', erp_id)])
        if extra_categ:
            for categ in extra_categ.extra_category_ids:
                cat_id = self.env['export.prestashop.categories'].with_context({'err_msg': '', 'create_ids': [], 'channel_id':self.channel_id}).sync_categories(
