        ...
        if channel_obj:
            for channel in channel_obj:
                if channel.auto_sync_stock:
                    ...
                    product_map_obj = channel._match_mapping(map_obj, domain)
                    if product_map_obj:
                        ps_product_id = product_map_obj.store_product_id
                        ps_variant_id = product_map_obj.store_variant_id
                        default_location_id = channel.location_id.id
                        if pick_details['location_dest_id'] == default_location_id:
                            product_qty = int(pick_details['product_qty'])
                        if pick_details['source_loc_id'] == default_location_id:
                            product_qty = int(-pick_details['product_qty'])
                        ...
                        if type(stock_search['stock_availables']) == dict:
                            ...
                            stock_data['stock_available']['quantity'] = int(stock_data['stock_available']['quantity'])+int(product_qty)
                            try:
